...
import { NativeBaseProvider, extendTheme, Text, Center, Spinner } from "native-base";
...
const Editor = ({ children, topics, onSave }: EditorProps) => {
	...
	const setCurrentFrame = useEditorStore(state => state.setDevice);
	const toggleCode = useEditorStore(state => state.toggleCode)
	const saveSourceCode = useEditorStore(state => state.saveSourceCode)
	const [sourceFile, setSourceFile] = useState<Source>();
	...
	var previousDiffs
	const { actions, query, connectors, enabled, hasChanges, currentEditorNodes } = useEditor((state, query) => {
	...
	// const onEditTheme = () => {
	// 	try {
	// 		notify({ action: 'selectNode', nodeId: 'theme-id' }, publish, () => actions.selectNode('theme-id'))
	// 	} catch (e) {
	// 		setError(`Error selecting theme`)
	// 		console.error(`Error selecting theme`)
	// 	}
	// }
	...
	// const devAction = () => { // CURRENT ACTION: gets current nodes
	// }
	...
	useKeypress(['z', 'Z', 's', 'S', 'c', 'C', 'Delete', 'Backspace'], (event) => {
		...
		else if ((event.key == "s" || event.key == "S") && (event.ctrlKey || event.metaKey)) { // Save
		...
		else if ((event.key == "c" || event.key == "C") && !userIsWritting && document.activeElement) {
			return toggleCode()
		}
		else if ((event.key == "Delete" || event.key == "Backspace") && document.activeElement) {
			// return actions.delete(currentNodeId)
		}
	});
	...
	// 		}
	// 	}
	// 	Object.keys(themeData?.data ?? []).forEach(d => {
	// 		if (!themeData.data[d] || !Object.keys(themeData.data[d]).length) {
	// 			delete themeData.data[d]
	// 		}
	// 	})
	// 	await EditorApi.updateTheme(themeData)
	// }
	...
	return (
		<div className={styles.content}>
			<Head>
				...
				<link rel="icon" type="image/png" sizes="16x16" alt="logo" href={require("../../assets/logos/logo.png")}></link>
				<style type="text/css">
					{currentFrame.scrollbar}
				</style>
			</Head>
			{/* <ModeController onSave={onSave} hasChanges={hasChanges && !error} /> */}
			...
			{/* <OptionsBar
				frames={frames}
				frameSelectionMenu={device}
				setFrameSelectionMenu={setCurrentFrame}
				selectedTheme={selectedTheme}
				onEditTheme={() => null}
				// onEditTheme={ENABLE_EDIT_THEME ? onEditTheme : null}
			/> */}
		</div>
	);
};
