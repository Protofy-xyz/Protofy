...
import Source, { resetTestUUID } from './Source';
    ...
    describe("Test JsxElement operations", () => {
        ...
        describe("Test JsxElement attributes", () => {
            ...
            it("obtains all specific attribute from a JsxElement", () => {
                ...
                expect(matchedAtribute1.getName()).toBe('atr1')
            })
        ...
        describe("Test transform from parent JsxElement all JsxExpression childs as JsxElemnt with tagName (ReactCode)", () => {
            it("transforms JsxExpressions to JsxElement with tagName (ReactCode)", () => {
                ...
                expect(newSource1.ast.getText()).toBe(`<View><View></View><View><Text atr={() => {}}>hello</Text></View><ReactCode codeBlock="{<MyComp atr={()=>{}}><Hello/></MyComp>}"/></View>`)
            })
        })
    })
    ...
    describe("Test JsxElement to CraftData (CraftJS) node", () => {
        it("generates craft node given node", () => {
            ...
            expect(craftData1).toStrictEqual({
                ...
                "isCanvas": true,
                "props": { "atr1": "1", "atr2": "{2}", "atr3": undefined, "atr4": "{{background:\"red\"}}" },
                "displayName": "View",
                "custom": {
            ...
            expect(craftData3).toStrictEqual({
                "type": {
                    "resolvedName": "ReactCode"
                },
                "isCanvas": true,
                "props": { "codeBlock": "{<Hello/>}" },
                "displayName": "ReactCode",
                "custom": {
        ...
        it("generates flatten list of craftNodes given node", () => {
            ...
            expect(tree).toStrictEqual({
                "nodeId-0": {
                    ...
                    "isCanvas": true,
                    "parent": "ROOT",
                    "props": { "atr1": "1", "atr2": "{2}", "atr3": undefined, "atr4": "{{background:\"red\"}}", "_nodeId": "nodeId-0", },
                    "displayName": "View",
                    "hidden": false,
        ...
        it("generates CraftJS nodes adding theme and ROOT node using 'toCraftNodes'", () => {
            ...
            resetTestUUID()
            const source2 = `<NativeBaseProvider><Background><Button></Button><View></View></Background></NativeBaseProvider>`
        ...
        it("dumps craftNodes back into source code", () => {
            const source1 = `<NativeBaseProvider theme={currentTheme}><View atr1="1" atr2={2} atr3 atr4={{background:"red"}} children={MY_CHILDREN}><Box atr1={()=>{console.log('hello')}} atr2={VARIABLE1}>{true?<Res/>:<Pas/>}</Box><Input children="protofie"/><Text>hello</Text></View></NativeBaseProvider>`
            const sourceFile1: Source = Source.parse(source1)
            let craftData1 = sourceFile1.data(true)
            const sourceFileTransformed1: string = sourceFile1.dump(craftData1)
            expect(sourceFileTransformed1).toBe(`<NativeBaseProvider theme={currentTheme}>
  <View atr1="1" atr2={2} atr3 atr4={{ background: "red" }}>
    {MY_CHILDREN}
    <Box
      atr1={() => {
...
`)
            resetTestUUID()
            // const source2 = `
            //     import {NativeBaseProvider} from 'native-base' import {View, Text} from 'react-native' import Box from 'baseapp/palettes/uikit/Box' import Input from 'baseapp/palettes/uikit/Input' export const HomeScreen = () => { return (<NativeBaseProvider theme={currentTheme}><View atr1="1" atr2={2} atr3 atr4={{"background":"red"}} children={MY_CHILDREN}><Box atr1={()=>{console.log('hello')}} atr2={VARIABLE1}>{true?<Res/>:<Pas/>}</Box><Input children="protofie"/><Text>hello</Text></View></NativeBaseProvider>)}`
            // const sourceFile2: Source = Source.parse(source2)
            // let craftData2 = sourceFile2.data(true)
            // const sourceFileTransformed2: string = sourceFile2.dump(craftData2)
            // const res = `import  { NativeBaseProvider } from 'native-base'\nimport  { View, Text } from 'react-native'\nimport Box from 'baseapp/palettes/uikit/Box'\nimport Input from 'baseapp/palettes/uikit/Input'\n\n                    export const HomeScreen = () => { return (<NativeBaseProvider theme={currentTheme}><View atr1="1" atr2={2} atr3 atr4={{"background":"red"}}>{MY_CHILDREN}<Box atr1={()=>{console.log('hello')}} atr2={VARIABLE1}>{true?<Res/>:<Pas/>}</Box><Input children="protofie"/><Text>hello</Text></View></NativeBaseProvider>)}`
            // expect(sourceFileTransformed2).toBe(res);
        })
        it("dump importDeclarations", () => {
            const source1 = `import React, { useState, useEffect } from 'react' import {NativeBaseProvider} from 'native-base' import {View, Text, Platform as MyPlatform} from 'react-native'import Box from 'baseapp/palettes/uikit/Box' import Input from 'baseapp/palettes/uikit/Input'
            ...
            craftData1["newNode"] = {
                ...
                custom: {
                    ...
                    moduleSpecifier: "'baseapp/palettes/uikit/MyAddedCustom'"
                },
                hidden: false,
                nodes: [],
                linkedNodes: {},
            ...
            craftData1['nodeId-1'] = {
                ...
                custom: {
                    ...
                    _nodeId: 'StringLiteral',
                    defaultImport: 'View',
                    moduleSpecifier: "'baseapp/palettes/uikit/View'"
                },
            }
            const newImportsContent = sourceFile1.dumpImportDeclarations(craftData1);
            expect(newImportsContent.replace(/[\t\n]/g, ' ')).toBe(`import React, { useState, useEffect } from 'react' import  { NativeBaseProvider } from 'native-base' import View from 'baseapp/palettes/uikit/View' import  { Text, Platform as MyPlatform } from 'react-native' import Box from 'baseapp/palettes/uikit/Box' import Input from 'baseapp/palettes/uikit/Input' import AddedComponent from 'baseapp/palettes/uikit/AddedComponent' `.replace(/[\t\n]/g, ' '))
        })
    })
})
