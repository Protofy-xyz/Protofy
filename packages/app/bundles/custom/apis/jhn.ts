...
export default Protofy("code", async (app, context) => {
  ...
  logger.info("IOT Router Jhn started");
  app.get("/api/v1/action", async (req, res) => {
    logger.info({}, "pup");
    await context.deviceAction("WX", "EDDED", "set_max_speed", 250);
    await context.deviceAction("WX", "EDDED", "set_target", 1750);
    res.send("Response");
  });
  app.get("/api/v1/release", async (req, res) => {
    logger.info({}, "pup");
    await context.deviceAction("WX", "EDDED", "hold_motor");
    res.send("Response");
  });
});
