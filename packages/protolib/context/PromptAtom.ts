...
export const PromptAtom = atom<PromptContext[]>([{
    ...
    generate: (prompt = '') => {
        ...
        The user can also modify apps/api or apps/adminpanel directly if specific customization is needed.
        ...
        - /workspace/services Allows to view resource usage for the runnings apps (like apps/core or apps/api). Its normal to see apps duplicated, but with the preffix -dev, since protofy start some apps (like apps/api and apps/core) twice, one in development mode and the other in production mode (compiled and without HMR)
