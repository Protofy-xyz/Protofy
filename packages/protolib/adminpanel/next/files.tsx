...
export default function FilesPage({data, pageSession}:any) {
  usePrompt(() => `At this moment the user is using a web file manager. The file manager allows to view and manage the files and directories of the project.
  ...
  Currently the user is in the directory: ${data?.CurrentPath}. The directory contents are: ${JSON.stringify(data?.filesState)}
  
  ${data?.CurrentFile?' viewing the file'+data?.CurrentFile+' is opened.':''}
  `) 
  return (
