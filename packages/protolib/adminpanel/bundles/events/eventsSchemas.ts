...
export const BaseEventSchema = z.object({
    ...
    user: z.string().hidden().generate((obj) => 'me').search(), // the original user that generates the action, 'system' if the event originated in the system itself
    payload: z.any().search(), // event payload, event-specific data
    //autogenerated by the create() method
    created: z.string().hidden().generate((obj) => moment().toISOString()).search(), // event date (iso)
    status: statusSchema,
    lastUpdated: z.string().hidden().generate((obj) => moment().toISOString()).search() //last event date (iso), the same as created if no updates were made
}) 
