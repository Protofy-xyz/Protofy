...
export const PromptAtom = atom<PromptContext[]>([{
    ...
    generate: (prompt) => {
        ...
        There is a custom bundle in packages/app/bundles/custom, where the user can add elements. The custom bundle is already wired with apps/api and apps/next. 
        ...
        The custom bundle (package/app/bundles/custom) includes two workspaces definitions at packages/app/bundles/custom/workspaces. One is admin.tsx and the editor is editor.tsx, with different control panel menus for both.
        ...
        By default, there are two predefined workspaces in the custom bundle (packages/app/bundles/custom): editor (in editor.tsx) and admin (in admin.tsx) in (both in packages/app/bundles/custom/workspaces), and they serve as a starting point. 
