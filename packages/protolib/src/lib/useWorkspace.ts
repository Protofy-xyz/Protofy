...
export const useWorkspace = (props?:{pages?:string[], boards?: string[]}) => {
    const {pages, boards} = props ?? {}
    ...
    return typeof workspaces[currentWorkspace] === 'function' ? workspaces[currentWorkspace]({ pages: pages ?? [], boards: boards ?? [] }) : workspaces[currentWorkspace]
}
