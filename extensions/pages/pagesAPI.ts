  ...
  const db = {
    ...
    async put(key, value) {
      ...
      try {
        ...
        // console.log('executing template: ', `/extensions/pages/templates/${template}.tpl`)
        const result = await API.post('/api/core/v1/templates/file?token=' + getServiceToken(), {
          name: fspath.basename(value.name + '.tsx'),
          data: {
            options: {
              template: `/extensions/pages/templates/${template}.tpl`,
              variables: {
                ...value,
                route: route,
      ...
      try {
        ...
        if (value.web) {
          ...
          const result = await API.post('/api/core/v1/templates/file?token=' + getServiceToken(), {
            ...
            data: {
              options: {
                template: `/extensions/pages/templates/nextPage.tpl`,
                variables: {
                  ...value,
      ...
      try {
        ...
        if (value.electron) {
          ...
          const result = await API.post('/api/core/v1/templates/file?token=' + getServiceToken(), {
            ...
            data: {
              options: {
                template: `/extensions/pages/templates/electronPage.tpl`,
                variables: {
                  ...value,
                  upperName: value.name ? value.name.charAt(0).toUpperCase() + value.name.slice(1) : ''
                }
